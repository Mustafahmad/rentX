...
public class uploadProperty extends AppCompatActivity {
                ...
                new ActivityResultContracts.StartActivityForResult(), new ActivityResultCallback<ActivityResult>() {
                    ...
                    public void onActivityResult(ActivityResult result) {
                        if (result.getResultCode()== Activity.RESULT_OK){
                            ...
                            Toast.makeText(uploadProperty.this,"No Image Selected", Toast.LENGTH_SHORT).show();
                        }
                    }
                }
        );
    ...
    private void saveData() {
        StorageReference storageReference = FirebaseStorage.getInstance().getReference().child("Property Category").child(uri.getLastPathSegment());
        AlertDialog.Builder builder = new AlertDialog.Builder(uploadProperty.this);
    ...
    public void uploadData(){
        ...
        propertyModels dataClass = new propertyModels(title,price,area,imageURL);

        String currentDate = DateFormat.getDateTimeInstance().format(Calendar.getInstance().getTime());
        FirebaseDatabase.getInstance().getReference("Property Category").child(currentDate)
                .setValue(dataClass).addOnCompleteListener(new OnCompleteListener<Void>() {
                    @Override
                    public void onComplete(@NonNull Task<Void> task) {
                        if (task.isSuccessful()){
                            Toast.makeText(uploadProperty.this, "Saved", Toast.LENGTH_SHORT).show();
                            finish();
                        }
                    }
                }).addOnFailureListener(new OnFailureListener() {
